...
def _validate_format(path: Path, artifact: dict[str, Any]) -> None:
    ...
    if fmt == "json":
        ...
        if path.stat().st_size % 4:
            raise MetadataError(f"{artifact['id']}: raw float32 file is not 4-byte aligned")
        handling = artifact.get("handling") or {}
        shape = handling.get("shape")
        if shape and path.stat().st_size != shape[0] * shape[1] * 4:
            raise MetadataError(f"{artifact['id']}: raw voice size does not match declared shape")
    elif fmt == "onnx":
        try:
            import onnx
        except ImportError:
...
def _iter_artifacts(registry: dict[str, Any], profile: str | None, artifact_id: str | None):
...
def _collect(registry: dict[str, Any], profile: str | None, artifact_id: str | None, update: bool) -> int:
    changed = False
    with tempfile.TemporaryDirectory(prefix="kokoro-registry-") as temporary:
        temporary_dir = Path(temporary)
        for model_id, distribution, artifact in _iter_artifacts(registry, profile, artifact_id):
            target = temporary_dir / model_id / distribution["id"] / artifact["local_name"]
            ...
            if artifact.get("size") is not None and artifact["size"] != size:
                raise MetadataError(f"{model_id}/{artifact['id']}: upstream size changed from {artifact['size']} to {size}")
            if artifact.get("sha256") is not None and artifact["sha256"] != digest:
                raise MetadataError(f"{model_id}/{artifact['id']}: upstream SHA-256 changed")
            print(f"  size={size} sha256={digest}")
            if update and (artifact.get("size") != size or artifact.get("sha256") != digest):
                artifact["size"] = size
...
def main(argv: list[str] | None = None) -> int:
    global REGISTRY
    parser = argparse.ArgumentParser(description="Collect runtime artifact sizes and SHA-256 values")
    parser.add_argument("profile", nargs="?")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--check", action="store_true", help="check committed metadata without downloading")
    mode.add_argument("--update", action="store_true", help="download and update metadata")
    ...
    try:
        ...
        if args.check:
            ...
            print(f"Checked {sum(len(d['artifacts']) for m in registry['models'].values() for d in m['distributions'])} committed runtime artifacts")
            return 0
