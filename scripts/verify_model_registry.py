...
def _validate_artifact(
    ...
    _require(
        all(field in artifact for field in required),
        f"{model_id}/{distribution_id}: artifact missing required field",
    )
    _require(
        isinstance(artifact["id"], str) and artifact["id"],
        f"{model_id}: artifact id is empty",
    )
    _require(
    ...
    _require(
        ...
        and artifact["size"] > 0,
        f"{model_id}: invalid size for {artifact['id']}",
    )
    _require(
        isinstance(artifact["sha256"], str)
        and SHA256_RE.fullmatch(artifact["sha256"]) is not None,
        f"{model_id}: invalid SHA-256 for {artifact['id']}",
    )
    role = artifact["role"]
    if role == "model":
        _require(
    ...
    if role in {"voice", "voices"} and artifact["format"] == "raw-float32-le":
        ...
        _require(
            handling.get("dtype") == "float32",
            f"{model_id}: raw voice {artifact['id']} needs float32 dtype",
        )
        _require(
            handling.get("endianness") == "little",
            f"{model_id}: raw voice {artifact['id']} needs little endianness",
        )
        ...
        _require(
            ...
            f"{model_id}: raw voice {artifact['id']} needs a positive 2D shape",
        )
    _require(
        "?" not in artifact["url"] or "resolve/" in artifact["url"],
...
def _validate_distribution(
    ...
    _require(
        distribution_id not in _distribution_ids,
        f"Duplicate distribution id: {distribution_id}",
    )
    ...
    for artifact in artifacts:
        _require(
            ...
            f"{model_id}/{distribution_id}: artifact is not an object",
        )
        _require(
        ...
        if artifact["role"] in {"voice", "voices"}:
            ...
            if artifact["role"] == "voice":
                _require(
                    bool(artifact.get("voice")),
                    f"{model_id}: voice artifact needs voice name",
                )
        ...
        if distribution["provider"] == "huggingface":
            ...
            _require(
                match is not None,
                f"{model_id}: Hugging Face artifact URL is not pinned: {artifact['url']}",
            )
            _require(
                match.group(1) != "main",
...
def verify_registry(
    ...
    _require(registry.get("schema") == 1, "Registry schema must be 1")
    _require(
        registry.get("runtime_contract") == 1, "Registry runtime contract must be 1"
    )
    release_entries = releases.get("releases")
    _require(
        isinstance(release_entries, dict), "Release catalog has no releases object"
    )
    ...
    for model_id, model in models.items():
        if model_id in {"v1.0", "v1.1-zh"}:
            ...
            _require(
                model["license"].get("source_repository")
                == release["source_repository"],
                f"{model_id}: license source repository is stale",
            )
            for distribution in model["distributions"]:
                provenance = distribution.get("provenance") or {}
                _require(
                    provenance.get("source_repository") == release["source_repository"],
                    f"{model_id}: distribution source repository is stale",
                )
                _require(
                    provenance.get("source_revision") == release["source_revision"],
                    f"{model_id}: distribution source revision is stale",
                )
        _require(
            FRONTEND_RE.fullmatch(model["frontend"]) is not None,
            f"{model_id}: frontend must be a machine-readable ID",
        )
        _require(
            all(LANGUAGE_RE.fullmatch(code) for code in model["language_codes"]),
        ...
        _require(
            ...
            f"{model_id}: duplicate distribution ID",
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Verify the Kokoro runtime model registry"
    )
    ...
    try:
        ...
        return 1
    print(
        f"Verified {len(registry['models'])} runtime models and {len(_distribution_ids)} distributions"
    )
    return 0
