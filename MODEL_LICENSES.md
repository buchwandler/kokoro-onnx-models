...
| Russian Zaakirio   | `zaakirio/kokoro-ru`                                            | The upstream model card declares model weights OpenRAIL and code Apache-2.0. Hugging Face metadata uses the generic `openrail` family identifier. Publication is disabled until the exact license text governing redistributed model and voice weights is identified, included with the release, and its redistribution obligations are reviewed. |
